...
const MODULE_VERSION = "2026-09-07-g83-18653-image-resubmit-live-v1.0.0";
const ROUTE = "/amazon/listing/g83-18653-image-resubmit-live";
const MARKETPLACE_ID = "A1VC38T7YXB528";
const PRODUCT_TYPE = "NOTEBOOK_COMPUTER";
const ISSUE_CODE = "18653";
const CONFIRMATION_TOKEN = "G83_18653_IMAGE_RESUBMIT_LIVE_APPROVED_20260907";
...
function buildGuardedLivePlan(sku, response) {
  ...
  if (!targetIssues.length) throw new Error(`FRESH_${ISSUE_CODE}_NOT_PRESENT ${sku}; LIVE_NOT_SENT`);
  ...
  if (!arraysEqual(keys, EXPECTED_IMAGE_KEYS)) {
    throw new Error(`IMAGE_KEY_SET_CHANGED ${sku} actual=${JSON.stringify(keys)}`);
  }
  ...
  if (requestBodySha256 !== target.previewRequestBodySha256) {
    throw new Error(`FRESH_PAYLOAD_HASH_MISMATCH ${sku} expected=${target.previewRequestBodySha256} actual=${requestBodySha256}; LIVE_NOT_SENT`);
  }
  ...
  return {
    ...
    lastUpdatedDate: summary.lastUpdatedDate || x.lastUpdatedDate || null,
...
async function livePatch(accessToken, sku, requestBody) {
  const { sellerId, marketplaceId, endpoint } = cfg();
  const q = new URLSearchParams({
    marketplaceIds: marketplaceId,
    issueLocale: "ja_JP",
...
async function handler(req, res) {
  try {
    ...
    if (confirmationToken !== CONFIRMATION_TOKEN) throw new Error("LIVE_CONFIRMATION_TOKEN_MISMATCH");

    const approvedSubmissionId = String(req.body?.approvedSubmissionId || "");
    const approvedRequestBodySha256 = String(req.body?.approvedRequestBodySha256 || "");
    if (approvedSubmissionId !== target.previewSubmissionId) throw new Error(`PREVIEW_SUBMISSION_ID_MISMATCH ${sku}`);
    if (approvedRequestBodySha256 !== target.previewRequestBodySha256) throw new Error(`PREVIEW_HASH_APPROVAL_MISMATCH ${sku}`);

    const accessToken = await token();
    const fresh = buildGuardedLivePlan(sku, await getListing(accessToken, sku));
    const live = await livePatch(accessToken, sku, fresh.requestBody);

    const liveIssues = Array.isArray(live.body?.issues) ? live.body.issues : [];
    const liveErrors = liveIssues.filter(i => String(i?.severity || "").toUpperCase() === "ERROR");
    const status = String(live.body?.status || "").toUpperCase();
    const accepted = live.ok && liveErrors.length === 0 && (status === "ACCEPTED" || status === "VALID");

    return res.status(live.ok ? 200 : 502).json({
      ok: accepted,
      ...
      guard: {
        ...
        issuePresentBeforeLive: true,
        imageOnly: true,
        exactEightImageKeys: true,
        previewSubmissionIdMatched: true,
        previewRequestBodySha256Matched: true,
        freshRequestBodySha256MatchedPreview: true,
      ...
      liveRequest: {
        patchPaths: fresh.patches.map(p => p.path),
        requestBodySha256: fresh.requestBodySha256,
      },
      liveResponse: {
        httpStatus: live.http,
        responseOk: live.ok,
        status,
        submissionId: String(live.body?.submissionId || ""),
        errorCount: liveErrors.length,
        issues: liveIssues,
      },
      accepted,
      amazonPersistentMutation: live.ok ? 1 : 0,
      ...
      adsMutation: 0,
      yahooMutation: 0,
      externalChanges: live.ok ? 1 : 0,
      note: "LIVE image-only resubmission. Issue clearance is asynchronous and must be confirmed by a later Fresh audit.",
    });
  } catch (err) {
    console.error("G83 18653 image resubmit LIVE error", err?.message || String(err));
    return res.status(400).json({
      ok: false,
      moduleVersion: MODULE_VERSION,
      route: ROUTE,
      amazonPersistentMutation: 0,
      variationRelationMutation: 0,
      priceMutation: 0,
      inventoryMutation: 0,
      b2bMutation: 0,
...
express.application.listen = function g8318653ImageResubmitLiveListen(...args) {
